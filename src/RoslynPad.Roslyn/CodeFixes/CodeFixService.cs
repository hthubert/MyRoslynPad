        ...
        internal class CodeFixService
        {
                ...
                CancellationToken cancellationToken)
            {
                ...
                await AppendFixesOrSuppressionsAsync(
                    ...
                    diagnostics,
                    result,
                    lazySuppressionProvider.Value,
                    hasFix: d => lazySuppressionProvider.Value.CanBeSuppressedOrUnsuppressed(d),
                    getFixes: async dxs => (await lazySuppressionProvider.Value.GetSuppressionsAsync(document, span, dxs, cancellationToken).ConfigureAwait(false)).AsImmutable(),
                    cancellationToken: cancellationToken).ConfigureAwait(false);
            }
                ...
                CancellationToken cancellationToken)
            {
                ...
                var fixAllProviderInfo = extensionManager.PerformFunction(fixer,
                    () => ImmutableInterlocked.GetOrAdd(ref _fixAllProviderMap, fixer, FixAllProviderInfo.Create),
                    defaultValue: null);

                FixAllState fixAllState = null;
                var supportedScopes = ImmutableArray<FixAllScope>.Empty;
                var diagnosticIds = GetFixAllDiagnosticIds(fixAllProviderInfo, diagnostics).ToImmutableHashSet();
                ...
                if (fixAllProviderInfo != null) {
                    var codeFixProvider = (fixer as CodeFixProvider) ?? new WrapperCodeFixProvider((ISuppressionFixProvider)fixer, diagnosticIds);
            ...
            private async Task<bool> ContainsAnyFix(Document document, DiagnosticData diagnostic, bool considerSuppressionFixes, CancellationToken cancellationToken)
            {
                ...
                var hasAnySharedFixer =
                    _workspaceFixersMap.TryGetValue(document.Project.Language, out var fixerMap)
                ...
                foreach (var fixer in allFixers) {
                    await extensionManager
                        .PerformActionAsync(fixer, () => fixer.RegisterCodeFixesAsync(context) ?? Task.CompletedTask)
